    ...
    if (jsonResponse != null && jsonResponse.hasKey("elements")) {
      ...
      for (int i = 0; i < elements.size(); i++) {
        ...
        if (element.hasKey("lat") && element.hasKey("lon")) {
          ...
          String amenityType = element.hasKey("amenity") ? element.getString("amenity") : "Unknown";
      ...
      if (locations.length > 0) {
        ...
        saveCSV("petrol_hospitals_tamilnadu.csv", locations);
        textSize(16);
        fill(0);
        text("CSV file with petrol stations and hospitals saved!", width / 2, height / 2);
      } else {
        textSize(16);
        fill(255, 0, 0);
        text("No petrol stations or hospitals found for Tamil Nadu.", width / 2, height / 2);
      }
    } else {
      textSize(16);
      fill(255, 0, 0);
